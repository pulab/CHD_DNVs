...
##input files read in
```{r}
motif_data <- read.table(file="../data/CHD_WT-ALT_fimo_all_value_with_motif_gene_fpkm1_new.txt",header = F)
...
```
```{r}
list_file <- read.table(file = "../data/CHD_active_fold_final_sig.txt",header = T,row.names = 1)
head(list_file)
list_merged_motif<-merge(motif_data,list_file,by.y="row.names",by.x="region")
head(list_merged_motif)
motif_data_sig<-list_merged_motif[list_merged_motif$final_sig !=0,]
```
## split IA DA and NC group
```{r}
...
write.table(plot_gt_ls6, "CHD_library_motif_changed_number.txt",sep="\t",quote = F)
```
## Got Odds ratio
```{r}
...
plot_gt_ls7[,c(1:2)]<- plot_gt_ls7[,c(1:2)]/41
plot_gt_ls7[,c(3:4)]<- plot_gt_ls7[,c(3:4)]/42
plot_gt_ls7[,c(5:6)]<- plot_gt_ls7[,c(5:6)]/388
...
plot_gt_ls57 <-plot_gt_ls7[rowMaxs(as.matrix(plot_gt_ls7[,c(1:4)]),value = T)>0.022,]
...
cols_sorted<-rev(rownames(plot_gt_ls57[cols$tree_row[["order"]],]))
...
motif_data[rownames(motif_data) %in% rownames(test_label),]$label<-test_label$gene_show
###
labels<- read.table("../data/CHD_scatter_plot_label.txt",header = T)
colnames(labels)<-c("motif_name","label_show")
labels <-labels[!is.na(labels$label_show),]
...
EMSA_list<-c("SRF_HUMAN.H11MO.0.A:::chr11:134277409-134277580:::GGGAGCGATCTCTCCTTTCTT:G",
        "TBX3_HUMAN.H11MO.0.C:::chr10:22555227-22555398:::T:C")
motif_data_sig_new$EMSA<-0
motif_data_sig_new[motif_data_sig_new$motif_name %in% EMSA_list,"EMSA"]<-1
sum(motif_data_sig_new$EMSA)

#list_merged_motif_full<-merge(motif_data,list_file,by.y="row.names",by.x="region")
motif_data_sig<-motif_data_sig_new[motif_data_sig_new$final_sig !=0,]
...
pdf("fig3f_CHD_all_sig_motif_changes_single_with_odd.pdf",width = 6,height = 6,useDingbats = F)
ggplot(motif_data_sig_final,aes(x=logPvalueWT,y=logPvalueALT,color=factor(change_2),shape=factor(EMSA),label=label_show,size=odds))+geom_point()+
