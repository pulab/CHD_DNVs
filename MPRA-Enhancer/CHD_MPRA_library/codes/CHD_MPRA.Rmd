...
inputfile<-"../data/CHD_MPRA_raw_counts_DNA1-4_RNA1-4_group.txt"
...
RNA=data[,c(5:8)]
DNA=data[,c(1:4)]
annot=data[,c(9:10)] %>% as.data.frame()
rownames(annot)<-rownames(DNA)
colnames(annot)<-c("groups","region_name")
...
inter_join <-split_for_pair(filter_norm_data_nc, vals = c("REF","ALT"),condition = "groups", 
                            group_by = "region_name",kept_annot = F, kept_res = c("Active"))
inter_join <- Diff_enhancers(inter_join,c(6:9),c(1:4))

inter_join$REF <- rowMeans(inter_join[,6:9])
...
ggplot(sub4_for_plot, aes(y=factor(group,levels=rev(c("GoF_REF","LoF_REF","GoF_ALT","LoF_ALT","Neg",
