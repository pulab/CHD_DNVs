...
## inputfiles ReadIN
```{r}
data<-read.table(file="../data/d17_readscount_data.txt",header=T,row.names = 1)
...
#write.table(DNA_total,"../data/figure1b_DNA_counts.txt",sep="\t",quote = F)
#dev.off()
```
...
#pdf(file="/Users/zhangxiaoran/Desktop/work/Bill/Fengxiao/mau/rev_1/rev2/violin_plot_of_d17_HHE_vs_Neg_new.pdf")
#pdf(file="/Users/zhangxiaoran/Desktop/work/Bill/Fengxiao/mau/rev_1/rev2/violin_plot_of_d17_HHE_vs_Neg_new_new.pdf")
...
ggplot(filter_norm_data_scaled_for_plot_new, aes(y = mean_log_ratio, x = factor(group4,levels=c("HHE_ATAC+",
  ...
  geom_boxplot(aes(fill =factor(group4)),color="white",width = 0.2)+theme_bw()
#dev.off()
#write.table(filter_norm_data_scaled_for_plot_new,"../data/figureS02d_d17_boxplot.txt",sep="\t",quote=F)
library(dplyr)
library(ggpubr)
...
```
```{r}
#write.table(dat_text,"../data/figure1c_d17_pvalue.txt",sep="\t",quote=F)
#write.table(filter_norm_data_scaled_grouped,"../data/figure1c_d17_rawdata.txt",sep="\t",quote=F)
#write.table(filter_norm_data_for_plot,"../data/figure1c_d17_enrichscore.txt",sep="\t",quote=F)
```
...
write.table(df_final_write,"../data/figure1c_d17_library_result.txt",sep="\t",quote = F)
...
#ilter_norm_data_scaled_grouped$avalue<-0
...
## d17 & d24 merged
```{r}
d24_filter_norm_data_nc_normalize_exp<-read.table(file="../data/d24_batch1-2__normalized_log2_ratio_add1.txt",header=T,row.names = 1)
d17_filter_norm_data_nc_normalize_exp <- read.table(file="../data/d17_batch1-2__normalized_log2_ratio_add1.txt",header=T,row.names = 1)
...
panel.cor <- function(x, y){
  ...
  text(0.5, 0.5, txt, cex = cex.cor*4 )
}
...
p<-corrplot(A_V_cor_merge, method  = "number",order = "alphabet",type="uper")
pheatmap(p$corr,cluster_cols=F, cluster_rows=F,col=colorRampPalette(c("blue","white","firebrick3"))(100),breaks=seq(0.5,1,0.005))
pheatmap(p$corr,cluster_cols=F, cluster_rows=F,col=colorRampPalette(c("blue","white","firebrick3"))(100))
...
scatter_plot_cor(d17_d24_filter_norm_data_nc_normalize_exp)
write.table(A_V_cor_merge,"../data/FigureS02b_d17_d24_merged_cor.txt",sep="\t",quote = F)
```
